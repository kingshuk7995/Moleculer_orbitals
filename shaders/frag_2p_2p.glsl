...
void main() {
    ...
    // Orbital radii for 2p orbitals
    float orbital_radius_fixed = 0.1;
    float orbital_radius_moving = 0.1;

    // Normalization factors for 2p orbitals
    ...
    // ψ for fixed atom (2p orbital aligned along x-axis)
    float base_psi_fixed = normalization_fixed * (dist_to_fixed.x / orbital_radius_fixed) * exp(-r_fixed / orbital_radius_fixed);
    ...
    // ψ for moving atom (2p orbital aligned along x-axis)
    float base_psi_moving = normalization_moving * (dist_to_moving.x / orbital_radius_moving) * exp(-r_moving / orbital_radius_moving);

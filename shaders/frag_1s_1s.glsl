...
void main() {
    ...
    float orbital_radius = 0.15;
    float normalization = 1.0 / sqrt(pi * pow(orbital_radius, 3.0));

    // ψ for fixed atom
    float base_psi_fixed = normalization * exp(-r_fixed / orbital_radius);
    ...
    // ψ for moving atom
    float base_psi_moving = normalization * exp(-r_moving / orbital_radius);
